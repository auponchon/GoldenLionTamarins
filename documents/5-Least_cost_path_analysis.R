...
roads<-sf::read_sf(here::here("data","RawData","Landscape","Roads","BRA_roads.shp")) 
...
landuse[landuse==2]<-10
landuse[landuse==3]<-20
landuse[landuse==4]<-50
landuse[landuse==5]<-100
...
loc<-read.table(here::here("data","rawData","Landscape","RegionsName.csv"),
  ...
  slice(1:10) %>% 
  sf::st_as_sf(., coords = c("Long", "Lat"), crs = 31983) 

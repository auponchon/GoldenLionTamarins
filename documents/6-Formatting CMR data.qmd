...
nb.gp.obs<-read.csv(here::here("data","NewlyCreatedData",
                             "Nb_obs_gp_include_cmr.csv"),
                  header=T,
                  sep=";") %>% 
  dplyr::filter(To.include==1 & !is.na(Group)) %>% 
  dplyr::select(Group)
...
format<-distinct.sum.up %>% 
 ...
 write.table(format, file=here::here("CMR", "data", "CMR_1state_AD_sex_filtGP_allperiods.inp"),
                 quote=F,row.names=F ,
                 sep=" ",
                 col.names=F)
 ...
 #same simple dataset but with all adults regardless sex
 sum.up<-data.clean.final %>% 
   dplyr::filter(!GLT %in% bad) %>% 
  dplyr::filter(Disp!=1 & Death=="0" & Solo=="0") %>% 
   dplyr::filter(IdadeOK=="AD") %>% #get only adults
...
format<-distinct.sum.up %>% 
 ...
 write.table(format, file=here::here("CMR", "data", "CMR_1state_AD_NOsex_filtGP_allperiods.inp"),
                 quote=F,row.names=F ,
                 sep=" ",
                 col.names=F)
```
 ...
 correct_translocation_cmr()  %>% 
  ...
  pivot_wider(id_cols=c(GLT,Juvenile,Subadult,Adult),
              ...
              names_prefix="Obs",
               values_from=State,
              values_fill=0) %>% 
    write.table(., file=here::here("Data","NewlyCreatedData","CMR", "CMR_3states-stage_nosex_allperiods.csv"),
                quote=F,row.names=F ,sep=";")

...
#  dplyr::filter(!is.na(IdadeOK)) %>% 
  ...
  dplyr::count(Year) %>% 
   dplyr::mutate(Male=ifelse(SexOK=="F","1","0"),
               Female=ifelse(SexOK=="M","1","0")) %>% 
  ungroup()
...
distinct.sum.up<-sum.up.correct %>% 
 correct_translocation_cmr()  %>% 
  dplyr::select(Year,GLT,State ) %>% 
  group_by(GLT,Year) %>% 
   slice_min(State) %>% 
  ungroup() %>% 
  distinct(Year,GLT,State) %>% 
  arrange(Year,GLT) %>% 
  pivot_wider(id_cols=GLT,
              names_from=Year,
              names_prefix="Obs",
               values_from=State,
              values_fill = 0) 
...
file<-"CMR_3states_AD_AldeiaI_II_1989-2007"
...
# Dataset with 2 stages and 2 states
...
dplyr::filter(!is.na(IdadeOK)) %>% 
  dplyr::filter(!is.na(BirthOK)) %>% 
   ...
   #                                    data.clean.final$Group=="AX2"])) %>% 
    dplyr::arrange(DateObs) %>% 
  group_by(GLT,Year,Group,UMMPs,SexOK,IdadeOK,BirthOK) %>% 
  dplyr::count(Year) %>% 
  dplyr::mutate(State=ifelse(IdadeOK=="JU" | 
                                IdadeOK=="SA" |
                                IdadeOK=="IN" ,1,
                      ifelse(IdadeOK=="AD",2,0))) %>% 
  ungroup()
...
distinct.sum.up<-sum.up.correct %>% 
 correct_translocation_cmr()  %>% 
  ...
  pivot_wider(id_cols=GLT,
   ...
   file<-"CMR_2states_2STAGES_1989-2022"
...
colNamesObs = colnames(distinct.sum.up)[2:ncol(distinct.sum.up)] #when stage is included

...
sub<-data.filter %>% 
  dplyr::filter(Group==gp[j])

ymdate<-sort(unique(sub$DateObs))

#loop through dates within group
if (length(ymdate) >1){
   for (k in 1:(length(ymdate)-1)){
 
  print(c(j,k))
        #group obs t   
sub1<-sub %>% 
  dplyr::filter(DateObs==ymdate[k])
short1<-sub1 %>% 
  select(Region,Group,GLT)

#group t+1
sub2<-sub %>% 
  dplyr::filter(DateObs==ymdate[k+1])
short2<-sub2 %>% 
  select(Region,Group,GLT)

#generate individuals 
if (!identical(short1,short2)){
  
  outsect<-setdiff(short2,short1)
  new1<-sub2 %>% 
    dplyr::filter(GLT %in% outsect$GLT) 
  
  newall<-rbind(newall,new1)
...
new<-newall %>% 
  group_by(GLT) %>% 
  slice_min(order_by = DateObs, n = 1) %>% 
  ungroup() %>% 
  dplyr::arrange(Group, DateObs)

 setTxtProgressBar(progress_bar(length(gp)),j)  

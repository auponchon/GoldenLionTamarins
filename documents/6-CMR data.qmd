...
#  dplyr::filter(ObsOrder>1) %>%    #remove first observation
  dplyr::mutate(Change=ifelse(ToGroup != FromGroup, 1,0)) %>% 
   ...
   dplyr::select(Year,DateObs,GLT,SexOK,
                 FromGroup,ToGroup,FromUMMP,ToUMMP,Change) %>% 
  ...
  dplyr::filter(!is.na(Change) & !is.na(Dist) & !is.na(FromGroupSize) ) %>% 
...
library(lme4)
library(effects)
gg<-glmer(Change ~ Outbreak + FromGroupSize +  ToGroupSize +(1|GLT) , data = data.disp, family = binomial)
